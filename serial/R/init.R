...
serialConnection<-function(name, port="com1", mode="115200,n,8,1", buffering="none", newline=0,eof="",translation="lf",handshake="none")
{
  ...
  # added so variable names will not have periods
  obj$portname<-gsub("[[:punct:]]", "", port)
...
open.serialConnection<-function(con, ...)
{
  ...
  tryCatch( .Tcl( paste("set sdev_",con$portname," [open ",os_path,con$port," r+]",sep=""))
            ,error = function(e) stop(simpleError(e$message))
  )
  ## setup configuration
  eof <- paste(" -eofchar ", con$eof,sep="")
  if(con$eof=="") eof=""
  .Tcl( paste("fconfigure $sdev_",con$portname
              ," -mode ", con$mode
...
close.serialConnection<-function(con, ...)
{
  if(isOpen(con))
  {
    tryCatch( .Tcl( paste( "close $sdev_",con$portname,sep = "" ) )
              ,error = function(e) stop(simpleError(e$message))
    )
    .Tcl( paste( "unset sdev_",con$portname,sep = "" ) )
  }
  invisible("DONE")
}
...
isOpen.serialConnection <-function(con,...)
{
  ...
  if(tclvalue( .Tcl( paste("info exists sdev_",con$portname, sep=""))) == 1) 
  {
    ...
    con_name <- tclvalue(paste("sdev_",con$portname, sep="")) 
